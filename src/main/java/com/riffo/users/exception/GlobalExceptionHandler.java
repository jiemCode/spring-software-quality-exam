...
public class GlobalExceptionHandler {
    ...
    public ResponseEntity<ApiErrorResponse> handleValidation(MethodArgumentNotValidException exception) {
        ...
        return buildResponse(HttpStatus.BAD_REQUEST, "La requete contient des donnees invalides", details);
    }
    ...
    private ResponseEntity<ApiErrorResponse> buildResponse(HttpStatus status, String message, List<String> details) {
        ApiErrorResponse body = new ApiErrorResponse(status.value(), status.getReasonPhrase(), message, details);
